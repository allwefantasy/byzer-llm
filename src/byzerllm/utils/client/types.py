...
class TemplateExpression(BaseModel):
    """
    Represents a template expression using jinja2 syntax.
    """
    template: str
    ...
    def render(self) -> str:
        """
        Render the template with variables using jinja2 syntax.
        Example:
            TemplateExpression(template="Hello {{ name }}", variables={"name": "World"})
            -> "Hello World"
        """
...
class TemplateRequest(BaseModel):
    """
    Represents a request containing multiple template expressions.
    """
    ...
    def render_all(self) -> List[str]:
        """
        Render all template expressions in the request.
        """
        return [expr.render() for expr in self.expressions]

class TemplateResponse(BaseModel):
    """
    Represents the response from template rendering.
    """
